...
```{r echo=FALSE, warning=FALSE, ,message=FALSE, results=FALSE}
Equipos <- read_csv("bases_de_datos/highest_earnings/highest_earning_teams.csv")
Jugadores <- read_csv("bases_de_datos/highest_earnings/highest_earning_players.csv")
Paises <- read_csv("bases_de_datos/highest_earnings/country.csv")
Juegos <- read_csv("bases_de_datos/historical/GeneralEsportData.csv")
Historico <- read_csv("bases_de_datos/historical/HistoricalEsportData.csv")
...
JugadoresIngresosT<- read_csv("bases_de_datos/players_teams/total_player_earnings.csv")
EquiposIngresosT <- read_csv("bases_de_datos/players_teams/total_team_earnings.csv")
```
...
Paises %>% rename (Continente = Continent_Name, Continente_codigo = Continent_Code, Pais = Country_Name, Pais_codigo_dos = Two_Letter_Country_Code, Pais_codigo_tres = Three_Letter_Country_Code, Numero_pais = Country_Number) -> PaisesN
...
JugadoresIngresosT %>% rename (Usuario = `Player ID`, Nombres = Name, Ingresos_totales = `Total Earning`, Equipo = Team) -> JugadoresIngresosTN

EquiposIngresosT %>% rename (Equipo = `Team Name`, Ingresos_totales = `Total Prize Won`) -> EquiposIngresosTN
```
...
# Países Ingresos (Decidir si los 5 NA's no deberían tomarse en cuenta)
...
#summary(PaisesIngresosN)

#Jugadores Ingresos Totales
JugadoresIngresosTN$Ingresos_totales = as.numeric(gsub("[\\$,]", "",  JugadoresIngresosTN$Ingresos_totales))
#summary(JugadoresIngresosTN)

#Equipos Ingresos Totales
EquiposIngresosTN$Ingresos_totales = as.numeric(gsub("[\\$,]", "",  EquiposIngresosTN$Ingresos_totales))
#summary(JugadoresIngresosTN)
```

# 1. Introducción

El tema que trata nuestra investigación es sobre la variación económica de los equipos y jugadores de E-sports en los Estados unidos entre los años 2019 y 2020. Para realizar un estudio más completo también se consideran los torneos internacionales en los que participaron jugadores y equipos estadounidenses.
...
El proyecto es factible debido a tres bases de datos obtenidas de la página de divulgación electrónica, Kaggle. Los datos sobre la cantidad de torneos y los premios monetarios son necesarios para obtener la información específica del dinero generado por cada equipo y jugadores durante los torneos en el periodo 2019-2020.

### 1.3.1 Limpieza de bases de datos

Para los fines de este estudio, en cada dataset se han eliminado resultados cuya información sea corrupta o no necesaria para el estudio. Por ejemplo: Equipos de Esport que no han participado en ningún torneo.
...
```{r}
mediaJugadores = floor(mean(HistoricoN$NumeroJugadoresGanadores, na.rm = TRUE))
mediaJugadores
```
...
```{r}
plot(JuegosN$Torneos, JuegosN$JugadoresGanadores, main = "CANTIDAD DE JUGADORES GANADORES POR TORNEO", sub = "CORRELACIÓN JUGADORES - TORNEOS", xlab = "Torneos", ylab = "Jugadores ganadores", xlim = c(1,100), ylim = c(1, 1000))
```
...
```{r}
plot(JuegosN$AñoLanzamiento, JuegosN$Premios, main = "MONTO TOTAL DE PREMIOS OFRECIDOS EN TORNEOS DESDE EL AÑO DE LANZAMIENTO", sub = "RELACIÓN ENTRE AÑO DE LANZAMIENTO Y PREMIOS" , xlab = "Año del juego", ylab = "Premio monetario", xlim = c(1990, 2021), ylim = c(1, 10000))
```
...
```{r}
hist(table(CompetitiveN$Equipo), breaks = 20, main = "CANTIDAD DE EQUIPOS GANADORES DE TORNEOS", sub = "TORNEOS POR CADA JUEGO", xlab = "Equipos", ylab = "Juegos")
```
